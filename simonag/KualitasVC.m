...
@property (strong, nonatomic) IBOutlet UITableView *kualitasTableView;
@property (strong, nonatomic) IBOutlet UILabel *jumlahBUMNdanJumlahProgram;

@end

@implementation KualitasVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    
    self.kualitasTableView.delegate = self;
    self.kualitasTableView.dataSource = self;
}
...
- (void)updateTableData {
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.kualitasTableView reloadData];
    });
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([dashboardList count] > 0 && [dashboardList count] > indexPath.row){
        if([[dashboardList objectAtIndex:indexPath.row] valueForKey:@"kualitas_persen"] != [NSNull null]) {
            cell.progressView.progress = [[[dashboardList objectAtIndex:indexPath.row] valueForKey:@"kualitas_persen"] floatValue] / 100;
        } else {
        ...
        if([[dashboardList objectAtIndex:indexPath.row] valueForKey:@"kualitas_persen"] != [NSNull null]) {
            ...
            cell.percentageValue.text = [NSString stringWithFormat:@"%@%%", [[dashboardList objectAtIndex:indexPath.row] valueForKey:@"kualitas_persen"]];
        } else {
...
- (void)viewWillAppear:(BOOL)animated {
    [self.kualitasTableView setUserInteractionEnabled:YES];
}
