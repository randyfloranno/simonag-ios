...
@property (strong, nonatomic) IBOutlet UITableView *komersialTableView;
@property (strong, nonatomic) IBOutlet UILabel *jumlahBUMNdanJumlahProgram;

@end

@implementation KomersialVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    
    self.komersialTableView.delegate = self;
    self.komersialTableView.dataSource = self;
}
...
- (void)updateTableData {
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.komersialTableView reloadData];
    });
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([dashboardList count] > 0 && [dashboardList count] > indexPath.row){
        if([[dashboardList objectAtIndex:indexPath.row] valueForKey:@"komersial_persen"] != [NSNull null]) {
            cell.progressView.progress = [[[dashboardList objectAtIndex:indexPath.row] valueForKey:@"komersial_persen"] floatValue] / 100;
        } else {
        ...
        if([[dashboardList objectAtIndex:indexPath.row] valueForKey:@"komersial_persen"] != [NSNull null]) {
            ...
            cell.percentageValue.text = [NSString stringWithFormat:@"%@%%", [[dashboardList objectAtIndex:indexPath.row] valueForKey:@"komersial_persen"]];
        } else {
...
- (void)viewWillAppear:(BOOL)animated {
    [self.komersialTableView setUserInteractionEnabled:YES];
}
